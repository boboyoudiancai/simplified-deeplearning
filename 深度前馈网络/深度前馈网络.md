...
w = tf.Variable(tf.random.normal([2, 1], seed=1))  
...
for epoch in range(1000):
    for j in range(4):
        x_sample = tf.expand_dims(X[j], axis=0)  
        y_sample = tf.expand_dims(Y[j], axis=0)
        with tf.GradientTape() as tape:
            y_pred = tf.matmul(x_sample, w) + b
            loss = tf.reduce_mean(tf.square(y_sample - y_pred))
        gradients = tape.gradient(loss, [w, b])
        optimizer.apply_gradients(zip(gradients, [w, b]))
    if epoch % 100 == 0:
        total_loss = tf.reduce_mean(tf.square(Y - (tf.matmul(X, w) + b)))
        print("step: %d, loss: %.3f" % (epoch, total_loss.numpy()))
print("X: %r" % X.tolist())
print("pred: %r" % (tf.matmul(X, w) + b).numpy().tolist())
```
...
weights = {
    'w1': tf.Variable(tf.random.normal([2, 16])),  # tf.random_normal → tf.random.normal
    'w2': tf.Variable(tf.random.normal([16, 1]))
}
bias = {
...
for i in range(max_step + 1):
    ...
    if i % 100 == 0:
        print('step: '+ str(i) + '  loss:' + "{:.6f}".format(loss.numpy()))
        # 保持原始预测输出格式（list嵌套显示）
        pred_np = nn(X, weights, bias).numpy()
        print(np.array2string(pred_np, formatter={'float_kind':lambda x: "%.6f" % x}))
print('end')
```

![xor_results](img/xor_result.jpg)
