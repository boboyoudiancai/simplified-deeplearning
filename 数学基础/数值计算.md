...
##定义真实参数
feature_num = 10
w_real = torch.randn(feature_num)  
b_real = torch.randn(1)            

##生成训练数据
instance_num = 1000
X = torch.randn(feature_num, instance_num)  
y = torch.matmul(w_real, X) + b_real        
y = y + 0.1 * torch.randn(y.shape)          

##初始化待训练参数
w = torch.randn(feature_num, requires_grad=True)  
...
```python
>>> w: [ 0.33987832  0.12520495  0.23770066  0.23349048 -1.1238787  -0.19048932
  2.2076435  -0.6366061   0.4661809   0.26420563]
>>> w_real: [ 0.33669037  0.1288094   0.23446237  0.23033303 -1.1228564  -0.18632829
  2.2082014  -0.63799703  0.46165723  0.26735088]
```
